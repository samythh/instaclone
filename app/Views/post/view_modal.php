   ...
   <div class="detail-info-side">
      ...
      <div class="detail-comments">
         <div class="comment-item">
            <a href="<?= site_url('profile/' . $poster['username']) ?>">
               <img
                  src="<?= base_url(empty($poster['profile_picture']) ? 'images/avatar.svg' : $poster['profile_picture']) ?>"
                  class="comment-avatar">
            </a>
            <div class="comment-content">
               <a href="<?= site_url('profile/' . $poster['username']) ?>" class="comment-username"
                  style="color: #262626; text-decoration: none;">
                  <?= esc($poster['username']) ?>
               </a>
         ...
         <?php foreach ($comments as $comment): ?>
            <div class="comment-item">
               <a href="<?= site_url('profile/' . $comment['commentername']) ?>">
                  <img
                     src="<?= base_url(empty($comment['profile_picture']) ? 'images/avatar.svg' : $comment['profile_picture']) ?>"
                     class="comment-avatar">
               </a>
               <div class="comment-content">
                  <a href="<?= site_url('profile/' . $comment['commentername']) ?>" class="comment-username"
                     style="color: #262626; text-decoration: none;">
                     <?= esc($comment['commentername']) ?>
                  </a>
      ...
      <div class="detail-footer">
         ...
         <div class="detail-likes-date">
            <div style="font-weight:600; font-size:14px; color:#262626; margin-bottom:4px;">
               <span id="likes-count-modal-<?= $post['post_id'] ?>"><?= esc($post['likes']) ?></span> suka
            </div>

            <div style="font-size:14px; color:#8e8e8e; margin-bottom:4px;">
